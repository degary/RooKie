...
class UserAdmin(BaseUserAdmin):
    ...
    def get_departments(self, obj):
        return (
            ", ".join([dept.name for dept in obj.departments.all()[:3]])
            or obj.department
            or "-"
        )
...
class DepartmentAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("基本信息", {"fields": ("name", "external_id", "parent_id", "source")}),
        ("配置信息", {"fields": ("order", "is_active")}),
            ...
            if obj and obj.config:
                ...
                try:
                    formatted_json = json.dumps(
                        obj.config, indent=2, ensure_ascii=False
                    )
...
class SystemModuleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("基本信息", {"fields": ("name", "display_name", "description", "icon")}),
        ("配置信息", {"fields": ("url_pattern", "is_active", "sort_order")}),
        ("时间信息", {"fields": ("created_at", "updated_at"), "classes": ("collapse",)}),
    )
...
class ModulePermissionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (
            "权限对象",
            {"fields": ("module", "user", "group"), "description": "用户和用户组只能选择其一"},
        ),
        ("权限设置", {"fields": ("can_view", "can_add", "can_change", "can_delete")}),
        ("时间设置", {"fields": ("expires_at",)}),
        ("系统信息", {"fields": ("granted_by", "granted_at"), "classes": ("collapse",)}),
    )
