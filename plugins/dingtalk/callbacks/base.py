...
class BaseCallbackHandler(ABC):
    ...
    def verify_signature(
        self, request: HttpRequest, nonce: str, timestamp: str, encrypt_msg: str
    ) -> str:
        """验证钉钉签名"""
        try:
            if not self.token:
    ...
    def dingtalk_response(self, message: str = "success") -> HttpResponse:
        ...
        return HttpResponse(
            json.dumps(encrypted_map), content_type="application/json", status=200
        )
    ...
    def _get_encrypted_map(self, contents: str) -> dict:
        ...
        signature = self._generate_signature(
            nonce, timestamp, self.token, encrypt_content
        )
    ...
    def _generate_signature(
        self, nonce: str, timestamp: str, token: str, msg_encrypt: str
    ) -> str:
