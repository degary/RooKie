...
class CallbackRouter:
    ...
    def get_handler(self, callback_type: str, config: Dict[str, Any]) -> Optional[object]:
        """获取回调处理器"""
    ...
    def route(self, request: HttpRequest) -> HttpResponse:
        ...
        try:
            ...
            if not all([signature, timestamp, nonce, encrypt_msg]):
                logger.error(f"缺少必要参数: signature={signature}, timestamp={timestamp}, nonce={nonce}, encrypt_msg={'***' if encrypt_msg else None}")
            ...
            # 验证签名
            expected_signature = temp_handler.verify_signature(request, nonce, timestamp, encrypt_msg)
