...
class ApiResponse:
    ...
    def __init__(
        self, success: bool, code: int, message: str, data=None, request_id: str = None
    ):
    ...
    def to_response(self) -> Response:
        """转换为DRF Response对象"""
        return Response(
            data=self.to_dict(), status=self.code if self.code < 600 else 200
        )
    ...
    def bad_request(cls, message: str = "请求参数错误", data=None):
        """请求参数错误"""
        return cls(
            success=False, code=ErrorCode.BAD_REQUEST, message=message, data=data
        )
    ...
    def validation_error(cls, message: str = "数据验证失败", data=None):
        """数据验证错误"""
        return cls(
            success=False, code=ErrorCode.VALIDATION_ERROR, message=message, data=data
        )
    ...
    @classmethod
    def business_error(
        cls, message: str, code: int = ErrorCode.OPERATION_FAILED, data=None
    ):
        """业务错误"""
        return cls(success=False, code=code, message=message, data=data)
